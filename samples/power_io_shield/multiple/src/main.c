...
static const struct device* power_io_shield_0 =
    DEVICE_DT_GET(DT_NODELABEL(power_io_shield0));
static const struct device* power_io_shield_1 =
    DEVICE_DT_GET(DT_NODELABEL(power_io_shield1));
...
int main() {
  if (!device_is_ready(power_io_shield_0)) {
    LOG_ERR("Power IO shield 0 not ready");
    return 1;
  }

  if (!device_is_ready(power_io_shield_1)) {
    LOG_ERR("Power IO shield 1 not ready");
    return 1;
  }
  ...
  for (;;) {
    for (size_t i = 0; i < ARRAY_SIZE(output_gpios) / 2; i++) {
      int ret = gpio_pin_set_dt(&output_gpios[i], (output_value >> i) & 1);
      if (ret != 0) {
        LOG_ERR("Failed to set output GPIO pin %d: %d", i, ret);
      }
      ret = gpio_pin_set_dt(&output_gpios[i + (ARRAY_SIZE(output_gpios) / 2)],
                            (output_value >> i) & 1);
      if (ret != 0) {
        LOG_ERR("Failed to set output GPIO pin %d: %d",
                i + (ARRAY_SIZE(output_gpios) / 2), ret);
      }
    }
    output_value++;
